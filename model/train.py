...
ap.add_argument('--dataset', type=str, default='ILICount', help="Dataset string")
#ap.add_argument('--dataset', type=str, default='COVIDCount', help="Dataset string")
ap.add_argument('--sim_mat', type=str, default='gravity', help="adjacency matrix filename (*-adj.txt)")
#ap.add_argument('--sim_mat', type=str, default='US-state-adj', help="adjacency matrix filename (*-adj.txt)")
ap.add_argument('--static', type=str, default='static_features', help="Static features-latitude,longitude,Population,Land Area in Sq. Mi.,Population Density(5*50) 5 features for 50 states")
ap.add_argument('--n_layer', type=int, default=1, help="number of layers (default 1)")
ap.add_argument('--n_hidden', type=int, default=5, help="rnn hidden states (could be set as any value)")
...
if args.mylog:
    ...
    #shutil.rmtree(tensorboard_log_dir) #TODO:throwing error
...
def evaluate(data_loader, data, tag='val'):
    ...
    for inputs in data_loader.get_batches(data, batch_size, False):
        ...
        #_,output,_  = model(X) #when recon loss used
        output, _ = model(X)
        #print("printing shape")
        #print(output.shape)
        #print(Y.shape)
        if output.shape == torch.Size([51]):
    ...
    #scale the predicted and true output values back to their original scales.
...
def train(data_loader, data):
    ...
    for inputs in data_loader.get_batches(data, batch_size, True):
        X, Y = inputs[0], inputs[1]
        optimizer.zero_grad()
        #recon_loss, output,_  = model(X) #when recon loss used
        output,_  = model(X) #The variable _ is used to store the hidden state of the RNN in this case
        #By convention, the underscore variable name is used to indicate that the value is not needed, but it still needs to be unpacked from the model's output, so it is captured in the variable.
        if Y.size(0) == 1:
            Y = Y.view(-1)
        loss_train = F.l1_loss(output, Y)# mse_loss
        '''when recon loss used
        #lambda_ = 1.5
        #batch_loss = loss_train + (lambda_ * recon_loss.mean() )
